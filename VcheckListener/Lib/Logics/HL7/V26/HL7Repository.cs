...
using VCheckListener.Lib.PMS;
using VCheckListener.Services;

namespace VCheckListener.Lib.Logics.HL7.V26
{
    public class HL7Repository
    {
            ...
            try
            {
                            ...
                            else
                            {
                                ...
                                else if (SubIDSplit[3] == "3")
                                {
                                    ...
                                    if (!obx.ObservationIdentifier.Text.Value.Contains(" IC"))
                                    {
                                        ...
                                        {
                                            TestParameter = obx.ObservationIdentifier.Text.Value,
                                            SubID = SubID,
                                            ProceduralControl = ProceduralControl,
                                            TestResultStatus = ResultStatus,
                                            TestResultValue = ResultValue,
                                            TestResultUnit = (!String.IsNullOrEmpty(ResultUnit)) ? ResultUnit : sResultRule,
                ...
                if (bResult)
                {
                    ...
                    if (sScheduledTestObj != null && !string.IsNullOrEmpty(sScheduledTestObj.ScheduleUniqueID))
                    {
                        ...
                        var success = await pmsHandler.SendToPMS(sTestResultObj, sTestResultDetails, sScheduledTestObj, sResultTestCode, notes);
                    ...
                    else if (PMS != null && !string.IsNullOrEmpty(PMS.ConfigurationValue))
                    {
        ...
        public static String GenerateNTEComments(List<NTE> nte)
        {
            ...
            {
                log.Error("GenerateNTEComments >>> V26 >>> ", ex);
            }
