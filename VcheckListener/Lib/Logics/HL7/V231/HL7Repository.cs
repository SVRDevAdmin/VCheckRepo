    ...
    class HL7Repository
    {
        ...
        public static async Task<bool> ProcessMessageAsync(NHapi.Base.Model.IMessage sIMessage, String sSystemName)
        {
            ...
            try
            {
                ...
                foreach (var observation in sORU_R01.PATIENT_RESULTs.FirstOrDefault().ORDER_OBSERVATIONs)
                {
                    ...
                    foreach (var observationDetail in observation.OBSERVATIONs)
                    {
                            ...
                            !(observationDetail.OBX.ObservationIdentifier.Identifier.Value.ToLower().Contains("title")))
                        {
                            ...
                            else { sStatus = General.ProcessObservationResultStatusValue(isRangeReference, sObservValue, observationDetail.OBX.ReferencesRange.Value, iResultValue); }
                ...
                sTestResultObj.DeviceSerialNo = Worker.MainModel.DeviceSerialNum != null && false ? Worker.MainModel.DeviceSerialNum : sSerialNo.Trim();
                ...
                if (bResult)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(sResultTestCode))
                        {
                        ...
                        var success = await pmsHandler.SendToPMS(sTestResultObj, sTestResultDetails, sScheduledTestObj, sResultTestCode);
            ...
            {
                log.Error("ProcessMessage >>> V231 >>> ", ex);
        ...
        public static String GenerateNTEComments(List<NHapi.Model.V231.Segment.NTE> nte)
        {
            ...
            {
                log.Error("GenerateNTEComments >>> V231 >>> ", ex);
            }
